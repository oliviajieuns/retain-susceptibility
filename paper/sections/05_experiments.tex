...
across parents: randomized sensitivity tracks the exact-norm ceiling
wherever the gradient family is deployed (RepNoise $2.63$ vs.\ $2.64$;
GradDiff $5.21$ vs.\ $4.93$, both far from the winning pool), so the
backward-free discount carries over to protection; and the guard holds
forgetting at criterion in every arm. The coherent
...
& randomized sensitivity (ours) & matched & yes & 1.898 & 5.212 \\
...
structure replicates under an independent seed -- every output-channel
optimum interior ($\alpha^{*}\in[0.25,0.75]$), every
representation-channel optimum at the corner. The separation between
per-channel $\alpha^{*}$ is the continuous analogue of the primary
interaction, and the deployed router is unchanged -- the declared
mapping remains $\alpha\in\{0,1\}$.
