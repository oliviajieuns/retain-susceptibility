...
\begin{equation}
    ...
    \label{eq:mixture}
\end{equation}
The endpoints recover the discrete router exactly -- $\alpha{=}1$ is the
randomized-sensitivity probe, $\alpha{=}0$ the proximity probe -- so
...
rather than a hard switch: output-channel parents map to high $\alpha$,
representation-channel parents to low $\alpha$, and hybrid objectives
...
mapping, fixed before any outcome is read (primary), or (ii) fit on
discovery folds only (adaptive variant), with the sealed audit untouched
in both cases. Second, prediction and protection are allowed different
