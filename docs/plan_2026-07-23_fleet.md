...
- **T6 — Qwen2.5-14B 제3 스케일 캠페인** (2026-07-23 결정: T1보다 우선 실행):
  ...
  **실측 (07-23 카나리아, run261216)**: 메모리는 문제없이 SFT 400스텝 완주.
  대신 **SFT 암기 게이트 미달** — NLL 400스텝에 0.961 (목표 0.8, 100스텝당 ~0.2
  하강). 원인: 학습 블록이 48층 중 마지막 8층뿐(7B는 28층 중 8층)이라 암기가
  느림. 조치: `sft_steps 400→800` (봉인 전 전제조건 수정, 목표 0.8 불변).
  800으로도 미달 시 다음 레버는 sft_lr 2e-5 — 그때도 이 문서에 기록할 것.
  **1.5B도 동일 현상 (07-23, run261225-ul3)**: 400스텝에 NLL 1.217 (100스텝당
...
- **1.5B 라운드2 판정 (07-23)**: graddiff는 lr 1.5e-6(reach .446/손상 in-bounds)과
  2e-6(reach .189/손상 폭발) 사이 급경사 → 라운드3 내부 탐색(lr175_s120,
  lr15_s180/240). rmu는 단조 반응(.77→.51→.42, 손상 ~0.005) → lr 1.6e-5/3.2e-5
  격상. **npo·simnpo는 lr×4·스텝×2에도 recall 정체(0.66~0.80), CB는 lr 무반응
  (0.848 고정), gru·repnoise는 도달 전 손상 폭발** — 1.5B 블록 용량에서는 대부분
  목적함수가 도달∧보존을 양립 못 한다는 실측. 라운드3 후에도 그대로면 1.5B 코어
  로스터를 graddiff·rmu(+도달 가능한 것)로 축소하고 나머지는 notreached로 문서화
  (audit 로스터 축소는 봉인 전 캠페인 설계 변경이라 허용).
- **7B 동결 블로커 (07-23)**: a199/circuit_breakers/a10_lr1_s240 셀 1개가 캠페인
  서명 없는 수동 런 → 격리(forensics) 후 fix1 유닛으로 재실행, 셀렉터 재실행.
- **1.5B 최종 판정 (07-23, 라운드3 종료 — 그리드 탐색 중단 확정)**: 3라운드
  25설정 실측으로 코어 7종 전부 "도달∧보존 공동 달성 불가".
  결정적 증거 — graddiff 파레토 전선이 허용 상자 밖: lr15_s240은 도달
  (recall .03/.04)하나 cvar 8.8/39.1로 폭발, lr175_s120은 a198만 통과
  (.085/0.99)하고 a199는 .254 미달 + mean 2.03 초과. rmu는 lr 1.6→3.2e-5에서
  recall .32→.29로 포화 (손상 ~0인데도 못 내려감). 저자 간 난이도 이질성
  (a199가 일관되게 어려움)도 실측. **처방: 1.5B audit은 영구 보류, 논문 1.5B
  증거는 기존 chanbal2 캠페인 + 이 도달불가 실측(스케일 결과로 §5 후보).**
  잔여 GPU는 7B/14B/Llama에 집중.
